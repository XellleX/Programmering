		...
		while(run)
		{
			...
			else
			{
				...
				while(s != summa)
				{
					if(svar.size() == 0)
					{
						System.out.println("Fel. Försök igen. " + fråga);
						svar.add(s);
					}
					...
					else
					{
						if(svar.contains(s))
						{
							System.out.println("Du har redan angett det svaret en gång. \n" + fråga);
						}
						else
						{
							System.out.println("Fel. Försök igen. " + fråga);
